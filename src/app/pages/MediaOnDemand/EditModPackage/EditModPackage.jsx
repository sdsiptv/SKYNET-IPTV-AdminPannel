...
export default function EditMODPackage() {
  ...
  const customList = (items, filteredData) => {
    const data = filteredData.length !== 0 ? filteredData : items;
    return (
      <Paper className={classes.paper_list}>
        <List dense component="div" role="list">
          {data.map(value => {
            const labelId = `transfer-list-item-${value?.mod_id}-label`;
            return (
              <ListItem
                key={value?.mod_id}
                role="listitem"
  ...
  const handleUpdatePackage = id => {
    let mod_id = [];
    mod_id = right.map(item => item.mod_id);
