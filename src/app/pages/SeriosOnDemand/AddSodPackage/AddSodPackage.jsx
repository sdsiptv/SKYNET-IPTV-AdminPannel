...
export default function AddSODPackage() {
  ...
  const [sodList, setSodList] = useState([]);
  ...
  const handleAddSODPackage = () => {
    let sod_id = [];
    for (var i = 0; i < sodList.length; i++) {
      for (var j = 0; j < right.length; j++) {
        if (right[j].title === sodList[i].title) {
          sod_id.push(sodList[i].sod_id);
        }
      }
    }
    setSods(sod_id);

    apis.addSodPackage(getValues('Name'), 1, sod_id).then(res => {
