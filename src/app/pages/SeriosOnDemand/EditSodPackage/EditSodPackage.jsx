...
export default function EditSODPackage() {
      ...
      <List dense component="div" role="list">
        {filteredData.length !== 0
          ? filteredData.map(value => {
              const labelId = `transfer-list-item-${value?.sod_id}-label`;
              ...
              return (
                ...
                >
                  ...
                  <ListItemText
                    id={labelId}
                    primary={value.sod_id + '-' + value.title}
                  />
                </ListItem>
              );
            })
  ...
  const handleGetSODMedia = () => {
    apis.getSodMedia().then(res => {
      let leftResult = res.data;

      apis
        .getDetailedSodPackage(location.state.state.data?.package_id)
        .then(res => {
          let rightResult = res.data;

          let filteredArray = leftResult.filter(
            leftChan =>
              rightResult.findIndex(
                rightChan => rightChan.sod_id === leftChan.sod_id,
              ) === -1,
          );
  ...
  const handleUpdateSODPackage = id => {
    let sod_id = [];
    sod_id = right.map(item => item.sod_id);
    apis
      .editSodPackage(
        getValues('Name'),
        getValues('enabled'),
              ...
              >
                <div style={{ paddingRight: '10px' }}>
                  <Button
                    ...
                    onClick={handleUpdateSODPackage}
                  >
                    Update
                  </Button>
                </div>
